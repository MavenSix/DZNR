...
```
/dznr build me an interactive native chat prototype for a fintech client called Levin Capital. Full product approach. Industry is fintech with a wealth management sub-vertical.
```
...
The `$ARGUMENTS` placeholder below is replaced with whatever the user typed after `/dznr`. The command body forwards the user's request to Tár via her plugin namespace.
